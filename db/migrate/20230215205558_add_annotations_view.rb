class AddAnnotationsView < ActiveRecord::Migration[6.1]
  def up
    execute <<-SQL
      CREATE OR REPLACE DEFINER = 'root'@'localhost' SQL SECURITY INVOKER VIEW annotations_and_data_entries AS
        select
          ...
          d.user_id,
          d.id as 'data_entry id', d.value,
          IF ((d.value REGEXP '^-?[0-9]+\\.?[0-9]*$'), CAST(d.value as DECIMAL(6,3)), null) as value_as_number,
          d.field_id,
