...
//global variables
  ...
  var countDiv; //used to add hidden fields to the form of a newly created annotation form. Facilitates selection of the correct form for jquery operations. Contents equal the html id "new_annotation" + current annotation count. Iterates on each new form creation
  var formDataCount; //used to select the correct annotation form input to place JSON data from transcription box form. Contents equal the html id "formData" + current annotation count. Iterates on each new form creation
...
//on transcription form submit, capture input as json and close the transcription box
  ...
  //call json grabber on form submit, put it in a box on the page. Converts form inputs to JSON. Adapted from http://jsfiddle.net/davidhong/gP9bh/
    $(function() {
      $('.transcription_form').submit(function(event) {
        event.preventDefault();//supress default submit action of refreshing the page, so we can add more than one annoation to the transcription
        ...
        //add attributes to the new form element that counlt's be added in the addForm() function due to the dash in the attribute name. Essential for model instance creation form.
...
//build form for each transcription submission
  ...
  function addForm() 
  //build form by appending elements to the container of "h4 Annotations", a div with id="annotateResults". Contains all the essential form elements and data for Rails model creation including an authenticity token. Rails variables passed to javascript by adding custom attributes such as "token" to html elements and grabbing their values using jquery. Modeled on the structure of another resulting html form created by rails functions in the "new.html.slim" file. In the "append" functions below, attributes are added as key: "value" pairs. Additional attributes for the form are added in the function that collects data from the transcription box after addForm() is called because they have illegal jquery chars (-) in the attribute name key and can only be passed as a string. 
  {
    ...
    var token = $('#annotateResults').attr('token'); //grab authenticity token value from the "token" attribute on the div with id="annotateResults". Next, add the hidden field that holds the token value to the form
    formDiv.append($("<input/>", 
                       { 
    ...
    //We're adding a form to create an annotation associated with a transcription that has not been created yet. In order to associate the newly created annotations with the future new transcription of the page we are on, the model instance ID of the last created transcription is passed to the forms for each entity tab in the transcription box. When the user clicks on the tab for that field group (entity), the form for that entity is given the ID "currentForm" to identify it as the active form. Upon clicking the submit button in the transcription box, we get the ID of the most recent transription using the ruby code "Transcription.last" and increment it by one to associate our new annotation with the next created transcription (the one we have yet to submit on the current page).
    var transcriptionId = $('#currentForm').attr('transcriptionCount');
    transcriptionId++;
    //Let's also get the ID of the entity related to the current form and the asset we are transcribing
    var entityId = $('#currentForm').attr('entityId');
    var assetId = $('#currentForm').attr('assetId');
    //Append hidden fields to have the form automatically associate the annotation with the current (future) transcription, the entity whose fields we are collecting data from, and the asset that we are transcribing.
    form.append(
       $("<input/>", 
           { 
             name:'annotation[entity_id]', 
             type:'hidden', 
             value: entityId,
    ...
    //Append the field that will hold the JSON string passed to the form by the json grabber function. Data in this field will be passed to the new annotation's :data attribute upon form submission
    form.append(
       $("<input/>", 
           { 
